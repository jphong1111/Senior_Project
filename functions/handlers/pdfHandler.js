...
function generateCalendarTable(startDate, events) {
    ...
    while (currentDate.getMonth() === startDate.getMonth()) {
        ...
        // currentDate.setDate(currentDate.getDate() + 1); // this will crash the program, interestingly. Don't uncomment unless you want to see!
    }
 ...
 *
 *
 * event -> an Event object representing the event whose invoice to generate
 * client -> a Client object representing the event's client
 * venue -> a Venue object representing the event's venue
 */
exports.generateInvoice = function (event, client, venue) {
    let content = [];
...
exports.generateArtistConfirmation = function (event, client, venue) {
    let content = [];
